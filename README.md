...
* Для отопления это возможность установить температуру, для гаража открыть\закрыть дверь, просмотр камеры видеонаблюдения, для освещения включен\выключен и может быть яркость, цвет.
...
Поддомен: Телеметрия
* Сбор и сохранение данных от датчика там где это имеет смысл. Для отопления это температура, для ворот состояние открыты\закрыты, для светильника включен\выключен.
